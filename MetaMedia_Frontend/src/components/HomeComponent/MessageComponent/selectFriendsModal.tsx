...
formData.append('image', data.image);
  ...
  const filteredFriends = friends.filter((user) =>
    user.basicInformation.fullName.toLowerCase().includes(searchText.toLowerCase())
  );
  ...
  <div className="w-full flex-col flex overflow-y-auto h-5/6 items-center">
    {filteredFriends.length > 0 ? (
      filteredFriends.map((user: any) => (
        ...
        style={{
          backgroundColor: selected.find(
            (selectedUser: any) => selectedUser.basicInformation.userId === user.basicInformation.userId
          ) ? '#ffe4e6' : 'transparent' // Change 'lightgray' to the desired background color
        }}
      >
          <div className="w-3/12 flex justify-center items-center">
            <img
              src={`https://meta-media.in/api/user/profile/${user?.profile?.profileUrl}`}
      ...
      <p>No matching friends found</p>
          ...
          <div className="w-full h-10 flex justify-center items-center"> <button className="text-sm border border-[#C1506D] p-2 h-7 w-14 flex justify-center items-center  font-semibold text-[#C1506D] rounded-md" onClick={handleCreate}>Create</button> </div>
          )}
        </div>
      </div>
    </div>
